...
let _providerOk: boolean | null = null;
...
async function checkProvider(): Promise<boolean> {
  if (_providerOk !== null) return _providerOk;
  try {
    const p = getProvider();
    ...
    _providerOk = typeof result === "number";
  } catch {
    _providerOk = false;
    if (_provider) {
...
export async function getWalletBalance(): Promise<WalletBalance> {
  ...
  if (!ok) return { matic: "N/A", usdc: "N/A", usdcRaw: 0n };
  ...
  try {
    ...
    return {
      matic: Number(ethers.formatEther(maticWei)).toFixed(4),
      usdc: (Number(usdc) / 1_000_000).toFixed(2),
      usdcRaw: usdc,
    };
  } catch {
    return { matic: "N/A", usdc: "N/A", usdcRaw: 0n };
  }
}
...
export async function executeTrade(opp: TradeOpportunity): Promise<TradeResult> {
  ...
  return config.trading.mode === "dry-run"
    ? executeDryRun(opp, timestamp)
    : executeLive(opp, timestamp);
}
...
async function executeLive(opp: TradeOpportunity, timestamp: string): Promise<TradeResult> {
  const ok = await checkProvider();
  if (!ok) {
    const error = `Cannot connect to Polygon RPC (${config.network.polygonRpc})`;
  ...
  if (isNaN(usdcAvailable) || usdcAvailable < opp.suggestedSizeUsd) {
    const error = `Insufficient USDC: have $${balance.usdc}, need $${opp.suggestedSizeUsd.toFixed(2)}`;
    console.error(`  [LIVE] ${error}`);
    return { opportunity: opp, mode: "live", success: false, error, timestamp };
  }

  if (!opp.token.tokenId) {
    const error = "No tokenId — cannot place order";
    console.error(`  [LIVE] ${error}`);
    return { opportunity: opp, mode: "live", success: false, error, timestamp };
  }
...
async function buildAndSubmitOrder(opp: TradeOpportunity): Promise<{ orderId: string }> {
  ...
  const orderMessage = {
    ...
    taker: ethers.ZeroAddress, tokenId: BigInt(opp.token.tokenId),
    makerAmount, takerAmount, expiration,
    nonce: 0n, feeRateBps: 0n, side: 0, signatureType: 0,
  };
  ...
  const res = await fetch(`${config.polymarket.clobApi}/order`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "POLY_ADDRESS": wallet.address },
    body: JSON.stringify({
      order: {
        salt: salt.toString(), maker: wallet.address, signer: wallet.address,
        taker: ethers.ZeroAddress, tokenId: opp.token.tokenId,
        makerAmount: makerAmount.toString(), takerAmount: takerAmount.toString(),
        expiration: expiration.toString(), nonce: "0", feeRateBps: "0",
        side: isBuyYes ? "BUY" : "SELL", signatureType: 0, signature,
      },
      owner: wallet.address, orderType: "GTD",
    }),
